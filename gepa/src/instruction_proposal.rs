...
fn render_value(value: &Reflective, level: usize) -> String {
    match value {
        Reflective::Map(entries) => render_sections(entries.iter().map(|(k, v)| (k.clone(), v)), level),
        Reflective::List(items) => {
            render_sections(items.iter().enumerate().map(|(i, v)| (format!("Item {}", i + 1), v)), level)
        }
...
/// there were none.
fn render_sections<'a>(entries: impl Iterator<Item = (String, &'a Reflective)>, level: usize) -> String {
...
fn skip_language_line(content: &str) -> &str {
    match content.find('\n') {
        Some(newline) if content[..newline].chars().all(|c| !c.is_whitespace()) => &content[newline + 1..],
        _ => content,
    }
}
...
mod tests {
    ...
    fn reflective_of(node: &Value) -> Reflective {
        ...
        if let Some(text) = object.get("text") {
            ...
            Reflective::Map(entries.as_array().expect("map entries").iter().map(pair_of).collect())
        } else {
            let items = object.get("list").expect("text, map, or list").as_array().expect("list items");
    ...
    fn pair_of(entry: &Value) -> (String, Reflective) {
        ...
        (pair[0].as_str().expect("key").to_string(), reflective_of(&pair[1]))
    }

    fn sample_of(sample: &Value) -> ReflectiveSample {
        sample.as_array().expect("a sample").iter().map(pair_of).collect()
    }
    ...
    fn renders_and_extracts_as_gepa_does() {
        let path =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/conformance/reflection.json");
        ...
        assert_eq!(
            ...
            fixture["default_template"].as_str().expect("default_template"),
            "the default template is byte-identical to gepa's"
        );

        for case in fixture["render_cases"].as_array().expect("render_cases") {
            let instruction = case["current_instruction"].as_str().expect("current_instruction");
            let dataset: Vec<ReflectiveSample> =
                case["dataset"].as_array().expect("dataset").iter().map(sample_of).collect();
