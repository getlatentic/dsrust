...
Two dependencies. Pin the exact version while DsRust is a pre-release.
...
DsRust asks the model over the network, so a call is `async` and your program needs a runtime.
Tokio is the only other crate you add.
