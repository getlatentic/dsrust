...
Every call is a network call, so every call is `async` and your program needs a runtime. The flavour is yours to choose — DsRust
...
The one exception is `typed_tool`, which reads a tool off a type you derive `JsonSchema` on
yourself; those derives are written in your crate, so that path — and only that path — asks for
`serde` and `schemars` in your manifest.

> **Status: 0.1.0, pinned to dspy 3.3.0.** DSPy's own suite tests the prompt bytes, the parsing, the
> optimizers and the RNG. The API is smaller than DSPy's and grows toward it ([Roadmap](#roadmap)).
