...
//!
//! The hardest third of the library, and where a port is most likely to go quietly wrong. Given
//! `{answer: "{}", "unknown: "7", reasoning": "[]"` the quotes no longer pair up, so the end of a
//! string has to be *decided* rather than read: by what follows the candidate quote, by which
//! container the cursor is in, by whether a colon appears before the next comma. These rules have
//! no specification other than the original's code, which is why this follows it line for line and
//! why the fixtures are generated by running it rather than written by hand.
