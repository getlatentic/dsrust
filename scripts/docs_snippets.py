...
def generate(guide: str) -> tuple[str, list[str]]:
    ...
    # dspy 3.3.1 deprecates `CodeAct` and `ProgramOfThought` in favour of `RLM`, and this crate
    # carries the same warning. A guide that documents them is documenting what upstream still
    # ships, so the allow is here rather than in every block that names one.
    header = (
        f"//! Generated by scripts/docs_snippets.py from {guide}. Edit the guide.\n"
        "#![allow(deprecated)]\n"
    )
