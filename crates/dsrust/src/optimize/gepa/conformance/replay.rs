//! GEPA's evolution against dspy's own, replayed decision for decision.
//!
//! Split from the scripted tests beside it because the two ask different questions: those drive the
//! wiring against a model written to make one instruction win, and this replays runs recorded from
//! dspy's GEPA teleprompter in the regime where the search dynamics decide — the reflection model
//! proposes a *distinct* instruction per ask, each with its own score profile, so whether a proposal
//! survives its seeded minibatch is what decides the pool, and different seeds compile different
//! instructions. What is compared is the whole evolution: every candidate in discovery order, its
//! parents, its validation score, the eval bookkeeping, the reflection-call count, and the winner.
...
use super::{GEPA, TABLE, marker, metric, trainset};
