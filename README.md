...
Today: **908 Rust tests pass**, and **891 of DSPy's own**, across 52 of its 86 test files. 479 of those cross into the Rust crate.
...
- [ ] **The rest of the sandbox.** DSPy's own interpreter suite runs against `DenoInterpreter`:
  33 of 44 pass, including the host-callback tools and the typed `SUBMIT`. The 11 left are declared
  by name — file mounting with write-back, the >100MB variable path, and three that no Rust
  interpreter can answer.
