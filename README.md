...
Today: **902 Rust tests pass**, and **879 of DSPy's own**, across 52 of its 86 test files. 479 of those cross into the Rust crate.
...
- [ ] **The rest of the sandbox.** `DenoInterpreter` runs Python today, and DSPy's own interpreter
  suite runs against it: 21 of 44 pass. The 23 left are declared by name and are four features —
  host-callback tools, a typed `SUBMIT` from the signature's output fields, file mounting with
  write-back, and the >100MB variable path.
