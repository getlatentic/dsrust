...
One dependency. Name the version: cargo does not pick a pre-release on its own.
...
Asking a model is a network call, so the call is `async`. `#[dsrust::main]` starts the runtime, so
you do not add one. Keep `#[tokio::main]` instead if your program already has tokio.
...
Today: **892 Rust tests pass**, and **858 of DSPy's own**, across 51 of its 86 test files. 441 of those cross into the Rust crate.
