...
//!
//! dspy seeds `random.Random(0)` and then draws from that one generator repeatedly: two
//! predictors filled in the same loop get different demos because the generator advanced between
//! them. Both halves are decisions a port has to keep — the fixed seed, and the advancing.
//!
//! Which examples an optimizer keeps *is* its output, and here that output is whatever these
//! draws select. So the generator and both of CPython's sampling algorithms are reproduced
//! exactly, and held to `tests/conformance/rng/cpython_random.json` — generated by running
//! CPython, never written by hand.
...
impl Rng {
    ...
    ///
    /// Rejecting rather than taking a remainder is what keeps the draw uniform, and the
    /// rejections are observable: a discarded draw still advances the generator, so a bound that
    /// wastes draws shifts every number that follows.
    ...
    /// CPython's small-population branch: offer each member once out of a pool, backfilling the
    /// hole from the tail so the untaken members stay contiguous without shifting.
