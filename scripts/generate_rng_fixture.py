...
# CPython's own `test_guaranteed_stable` (Lib/test/test_random.py, MersenneTwister_TestBasicOps):
# values "guaranteed to stay the same across versions of python". Copied from CPython's source, not
# generated by running the interpreter — so this anchors `random()` (the 53-bit double) against
# CPython's published known answer rather than against the local build. `seed(3456147, version=1)`
# is `Random(3456147)` for an int seed (version only changes str/bytes seeding), which is exactly
# how this crate seeds. Held to the bit.
CPYTHON_GUARANTEED_STABLE = {
    "seed": 3456147,
    "random_hex": [
        "0x1.ac362300d90d2p-1",
        "0x1.9d16f74365005p-1",
...
def double_bits(value: float) -> str:
    """A double's raw little-endian IEEE-754 bits, as a decimal string — the exact representation, so
    a JSON round-trip cannot shift the last bit."""
...
def main() -> None:
    ...
    fixture = {
        ...
        # CPython's own guaranteed-stable known answer for `random()`, as raw double bits.
        "cpython_guaranteed_stable": {
            "seed": CPYTHON_GUARANTEED_STABLE["seed"],
            "note": "copied from CPython Lib/test/test_random.py test_guaranteed_stable",
            "random_bits": [double_bits(float.fromhex(h)) for h in CPYTHON_GUARANTEED_STABLE["random_hex"]],
        },
    }
