...
the `ChatModel` trait. Compose and optimize — `ReActV2!` is the same call for an agent whose model
calls tools natively:

```rust
let agent = ReAct!("question -> answer", tools);          // thought, tool, observation
...
Two layers, both against the pinned upstream (`dspy==3.3.1`, `gepa==0.1.4`, `optuna==4.9.0`),
...
<!-- status: generated by scripts/status.py from scripts/status.toml -->

Today: **1806 Rust tests pass**, and **1316 of DSPy's own**, across 69 of its 97 test files. 808 of those cross into the Rust crate.
...
[`dsrust-harness`](crates/dsrust-harness) — coding agents as dsrust models. `HarnessModel` puts
any [agent-harness](https://crates.io/crates/agent-harness) adapter — Claude Code, Codex, an ACP
agent, the built-in OpenAI-compatible runtime — behind `ChatModel`, so a `Predict`, a
...
offered to the agent as an MCP server living in your process. Not yet published.

---

## Roadmap

The byte and algorithm levels are strong, verified against DSPy's own tests. Every public symbol
DSPy defines in a ported module is mapped, justified, or tracked — 100% across its top-level names,
its methods, and their parameters.

**Done**

- [x] **Cloudflare Workers Phase 1**: text inference, HTTP providers, tools, streaming, retries,
  tracing, scoped state, and request-owned memory caching. See
...
- [x] **Optimizers**: LabeledFewShot, BootstrapFewShot, BootstrapFewShotWithRandomSearch,
  BootstrapFewShotWithOptuna, COPRO, MIPROv2, GEPA (reflective mutation *and* merge, with CPython
  set-order and RNG reproduced), SIMBA, InferRules, KNNFewShot, Ensemble, BetterTogether.
...
- [x] **Custom types**: `dspy.Type` with `Image`, `Audio`, `File`, `Code`, `Citations`, `Document`.
- [x] **Sandbox**: DSPy's own `runner.js` under Deno and Pyodide, defaulted the same way.
- [x] **Retrieval**: `Embedder`, `KNN`, `Embeddings`, numpy's own `argsort` and `.npy` format.
- [x] **Observability**: all six of DSPy's callback points as a trait, and a `tracing` span at each.
- [x] **Error taxonomy**: `LmErrorKind`, litellm's own context-window phrases, retry classification.

**Next**

- [ ] **dspy 3.4**: the pin moves again when a release lands. `CodeAct` and `ProgramOfThought` are
  deprecated upstream in favour of `RLM`, and carry the same warning here.

**Planned**

- [ ] **Cloudflare Workers Phases 2–4**: Workers AI and KV, then image/audio inputs, followed by
  Worker-native retrieval and persistence integrations.
- [ ] Finetuning and the RL optimizers (`BootstrapFinetune`, `GRPO`), which are out of the 1.0
  ceiling today.
