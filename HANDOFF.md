...
A from-scratch Rust port of Python **DSPy**, pinned to **dspy 3.3.0** (`scripts/DSPY_VERSION`,
...
| | |
|---|---|
| Rust tests | 1676 passing |
| Upstream dspy tests | 1215 passing, 730 crossing into Rust, 687 deciding a signature |
| Upstream files run | 69 of 94, every other one excused by name |
| Strict-xfail backlog | 8 entries in `conftest.py` |
...
The submodule is 3.3.0 and every gate runs against it, which is right — a moving target cannot be
...
**Main moves past the pin.** 3.3.0 is the tag this tree is held to; 3.3.1 exists and is being ported
on its own branch. So "ahead of the pin" does not mean "wrong", and the reason to hold is
...
```sh
git -C third_party/dspy diff --stat 3.3.0..origin/main -- dspy/
```
