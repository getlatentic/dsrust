...
/// The prefixes a buffer may end with while still becoming `[[ ## ` — upstream's
/// `end_pattern_prefixes`.
const END_PREFIXES: [&str; 5] = ["[", "[[", "[[ ", "[[ #", "[[ ##"];
...
impl FieldListener {
    ...
    pub fn push(&mut self, delta: &str) -> Option<FieldChunk> {
        ...
        if !self.started
            && let Some(at) = delta.find(&self.start)
            && delta[at + self.start.len()..].contains(MARKER)
        {
            self.started = true;
            self.ended = true;
            return None;
    ...
    fn opening(&mut self, delta: &str) -> Option<String> {
        ...
        if !ends_with_prefix_of(concat.trim(), &self.start) {
            self.opening.clear();
        }
        None
    }
    ...
    fn emit(&mut self, delta: String) -> Option<FieldChunk> {
        ...
        if held(&self.pending).trim().contains(MARKER) {
            self.ended = true;
...
/// Whether the buffer could still be growing into a closing marker — upstream's
/// `_could_form_end_identifier`, which is what lets an ordinary token be released at once instead
/// of waiting out the ten-delta window.
fn could_close(buffered: &str) -> bool {
    END_PREFIXES.iter().any(|prefix| buffered.ends_with(prefix)) || buffered.contains(MARKER)
}
