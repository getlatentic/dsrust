...
- **Watch a run with the tracing you already have.** Every module and every model call is a span,
  nested as the program is nested. `RUST_LOG=dsrust::observe=info` and nothing else.
- **Or skip the signature entirely.** `lm.call(items![User(["…"])])` is DSPy's `lm(dspy.User(...))`:
  a string, a conversation, or a previous reply handed straight back in for the next turn.
...
Today: **989 Rust tests pass**, and **897 of DSPy's own**, across 52 of its 86 test files. 479 of those cross into the Rust crate.
...
- [ ] **The other four observation points**: adapter format, adapter parse, tool and evaluate.
  Module and lm are spans today; DSPy fires a callback at all six.
