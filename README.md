...
Today: **902 Rust tests pass**, and **858 of DSPy's own**, across 51 of its 86 test files. 441 of those cross into the Rust crate.
...
- [ ] **The sandbox, held to DSPy's own interpreter suite.** `DenoInterpreter` runs Python today —
  it executes DSPy's own vendored `runner.js` under Deno and Pyodide, so a model's code lands in the
  same sandbox. What is left is running upstream's 44 interpreter tests against it, which is what
  turns "it works" into "it agrees".
