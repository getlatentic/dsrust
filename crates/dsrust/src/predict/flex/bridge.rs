...
use crate::interpreter::{CodeInterpreter, Executed};
...
impl Bridged {
    fn signature(&self) -> &Signature {
        match self {
            Bridged::Predict(predict) => &predict.signature,
            Bridged::ChainOfThought(chain) => chain.signature(),
        }
    }
...
impl Built {
    ...
    fn construct(&mut self, args: &Value) -> Result<Value> {
        ...
        let predictor = match kind {
            ...
            other => bail!(
                "dspy.Flex: the generated code built a `{other}` for `{attribute}`, and this crate \
                 bridges `Predict` and `ChainOfThought` so far"
            ),
        };
    ...
    async fn call(&self, args: &Value) -> Result<Value> {
        ...
        Ok(fields_of(&answered, predictor.signature()))
    }
}

/// Only the signature's own outputs cross back, which is what the generated `forward` reads.
fn fields_of(prediction: &Prediction, signature: &Signature) -> Value {
    let fields: Map<String, Value> = signature
        .outputs
        .iter()
        .filter_map(|field| {
            prediction
